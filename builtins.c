...
void throw_error(query *q, cell *c, const char *err_type, const char *expected)
{
	...
	tmp.nbr_cells = 1;
	tmp.arity = 0;
	size_t len = write_term_to_buf(q, NULL, 0, &tmp, 1, 0, 0, 0);
	char *dst = malloc(len+1);
	write_term_to_buf(q, dst, len+1, &tmp, 1, 0, 0, 0);
...
static int fn_iso_atom_concat_3(query *q)
{
	...
	if (is_var(p3)) {
		...
		if (is_atom(p1)) {
			...
			write_term_to_buf(q, tmpbuf1, sizeof(tmpbuf1), p1, 1, 0, 0, 0);
			len1 = strlen(tmpbuf1);
			src1 = tmpbuf1;
		}
		...
		if (is_atom(p2)) {
			...
			write_term_to_buf(q, tmpbuf2, sizeof(tmpbuf2), p2, 1, 0, 0, 0);
			len2 = strlen(tmpbuf2);
			src2 = tmpbuf2;
		}
...
static int fn_iso_open_4(query *q)
{
	...
	if (is_var(mmap_var)) {
		...
		tmp.flags |= FLAG2_CONST_STRING|FLAG2_PRETTY_PRINT;
		tmp.nbr_cells = 1;
		tmp.val_str = addr;
		tmp.len_str = len;
		tmp.rem_str = len;
...
static int fn_iso_write_1(query *q)
{
	...
	int n = get_named_stream(q, "user_output");
	stream *str = &g_streams[n];
	write_term_to_stream(q, str, p1, 1, 0, 200, 0);
	return !ferror(str->fp);
}
...
static int fn_iso_writeq_1(query *q)
{
	...
	int save = q->quoted;
	q->quoted = 1;
	write_term_to_stream(q, str, p1, 1, 0, 200, 1);
	q->quoted = save;
	return !ferror(str->fp);
}
...
static int fn_iso_write_term_2(query *q)
{
	...
	q->latest_ctx = p1_ctx;
	write_term_to_stream(q, str, p1, 1, 0, q->max_depth, q->quoted?1:0);
...
static int do_throw_term(query *q, cell *c)
{
	...
	write_term(q, stderr, c, 1, 0, 999, 0);
	fprintf(stderr, "\n");
	q->exception = NULL;
	q->error = 1;
	return 0;
...
static int fn_term_to_atom_2(query *q)
{
	...
	if (is_number(p1)) {
		...
		write_term_to_buf(q, dst, sizeof(tmpbuf), p1, 1, 0, 999, 0);
		cell tmp = make_cstring(q, dst);
		return unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	}

	size_t len = write_term_to_buf(q, NULL, 0, p1, 1, 0, 999, 0);
	char *dst = malloc(len+1);
	write_term_to_buf(q, dst, len+1, p1, 1, 0, 999, 0);
	idx_t offset;
	...
	while (*src) {
		...
		if (ch == 'c') {
			...
			if (ch == 'q')
				...
				c->flags |= FLAG2_PRETTY_PRINT;

			if (canonical)
				len = write_canonical_to_buf(q, NULL, 0, c, 1, 0);
			else
				len = write_term_to_buf(q, NULL, 0, c, 1, 0, 999, 0);
			...
			if (canonical)
				...
				len = write_term_to_buf(q, dst, nbytes, c, 1, 0, 999, 0);
...
static int fn_urlenc_2(query *q)
{
	...
	if (is_atom_or_var(p1) && is_var(p2))
		return do_urlencode_2(q);
	else if (is_var(p1) && is_atom_or_var(p2))
