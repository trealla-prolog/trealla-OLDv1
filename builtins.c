...
static void stream_assert(query *q, int n)
{
	...
	dst += snprintf(dst, sizeof(tmpbuf)-strlen(tmpbuf), "'$stream_property'(%s, end_of_stream(%s)).\n", str->name, str->past_end_of_file ? "past" : str->at_end_of_file ? "at" : "not");
...
static USE_RESULT prolog_state fn_split_4(query *q)
{
	...
	if ((ptr = strchr_utf8(start, ch)) != NULL) {
		...
		if (*ptr)
			may_error(make_string(&tmp, ptr, LEN_STR(p1)-(ptr-start)));
		else
			make_literal(&tmp, g_nil_s);
...
static USE_RESULT prolog_state fn_absolute_file_name_3(query *q)
{
	...
	if (is_string(p1))
		...
		may_error(make_cstringn(&tmp, tmpbuf, strlen(tmpbuf)), free(tmpbuf); free(src));
