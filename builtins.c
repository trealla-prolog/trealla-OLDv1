...
static cell make_cstringn(const char *s, size_t n)
{
	cell tmp;
...
static int fn_iso_atom_chars_2(query *q)
{
	...
	if (is_string(p2)) {
		cell tmp = make_cstringn(GET_STR(p2), LEN_STR(p2));
		return unify(q, p1, p1_ctx, &tmp, q->st.curr_frame);
	}
...
static int fn_iso_atom_concat_3(query *q)
{
	...
	if (is_variable(p3)) {
		...
		dst[nbytes] = '\0';
		cell tmp = make_cstringn(dst, nbytes);
...
static int do_throw_term(query *q, cell *c)
{
	...
	write_term(q, stderr, c, c_ctx, 1, 0, 0);
	fprintf(stdout, "\n");
	q->m->dump_vars = 1;
	q->exception = NULL;
	q->error = true;
...
static int fn_iso_current_prolog_flag_2(query *q)
{
	...
	if (!strcmp(GET_STR(p1), "double_quotes")) {
		...
		int i = g_avc;
		cell tmp = make_cstring(g_av[i++]);
		alloc_list(q, &tmp);

		while (i < g_ac) {
			tmp = make_cstring(g_av[i++]);
			append_list(q, &tmp);
		}
...
static int fn_split_atom_4(query *q)
{
	...
	while ((ptr = strchr_utf8(start, ch)) != NULL) {
		...
		cell tmp = make_cstringn(start, ptr-start);
...
static int fn_getfile_2(query *q)
{
	...
	while (getline(&line, &len, fp) != -1) {
		...
		cell tmp = make_cstringn(line, len);
...
static int fn_getenv_2(query *q)
{
	...
	cell tmp = make_cstring((char*)value);
	return unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
}
