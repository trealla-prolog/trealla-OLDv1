...
static void chk_for_tmp_cstring(cell *c)
{
	if (is_tmp(c)) {
		FREE_STR(c);
...
static void chk_struct_for_tmp_cstrings(cell *s)
{
	cell *c = s;

	for (idx_t i = 0; i < s->nbr_cells; i++, c++)
		chk_for_tmp_cstring(c);
}
...
static USE_RESULT pl_state fn_iso_stream_property_2(query *q)
{
	...
	if (!match_clause(q, tmp, q->st.curr_frame, DO_CLAUSE)) {
		chk_struct_for_tmp_cstrings(tmp);
		clear_streams_properties(q);
