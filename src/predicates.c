...
static USE_RESULT pl_status fn_iso_halt_1(query *q)
{
	...
	q->halt_code = get_denominator(p1);
	q->halt = q->error = true;
	return pl_halt;
}
...
static int get_stream(query *q, cell *p1)
{
	...
	if ((get_numerator(p1) < 0) || (get_numerator(p1) >= MAX_STREAMS)) {
		//DISCARD_RESULT throw_error(q, p1, "type_error", "stream");
		return -1;
	}

	if (!g_streams[get_numerator(p1)].fp) {
		//DISCARD_RESULT throw_error(q, p1, "existence_error", "stream");
		return -1;
	}

	return get_numerator(p1);
}
...
static USE_RESULT pl_status fn_iso_set_stream_position_2(query *q)
{
	...
	if (fseeko(str->fp, get_numerator(p1), SEEK_SET))
...
static pl_status do_stream_property(query *q)
{
	...
	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "file_name")) {
		cell tmp;
	...
	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "alias")) {
		cell tmp;
	...
	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "mode")) {
		cell tmp;
	...
	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "type")) {
		cell tmp;
	...
	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "reposition")) {
		cell tmp;
	...
	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "input"))
		return !strcmp(str->mode, "read");

	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "output"))
		return strcmp(str->mode, "read");

	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "eof_action") && is_stream(pstr)) {
		cell tmp;
	...
	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "end_of_stream") && is_stream(pstr)) {
		bool at_end_of_file = false;
	...
	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "position") && !is_variable(pstr)) {
		cell tmp;
		make_int(&tmp, ftello(str->fp));
		return unify(q, c, q->latest_ctx, &tmp, q->st.curr_frame);
	}

	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "line_count") && !is_variable(pstr)) {
		cell tmp;
...
static USE_RESULT pl_status fn_iso_open_4(query *q)
{
	...
	if (is_structure(p1) && (p1->arity == 1) && !slicecmp2(GET_STR(p1), LEN_STR(p1), "stream")) {
...
static bool parse_read_params(query *q, parser *p, cell *c, cell **vars, idx_t *vars_ctx, cell **varnames, idx_t *varnames_ctx, cell **sings, idx_t *sings_ctx)
{
	...
	if (!slicecmp2(GET_STR(c), LEN_STR(c), "character_escapes")) {
		if (is_literal(c1))
			p->flag.character_escapes = !slicecmp2(GET_STR(c1), LEN_STR(c1), "true");
	} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "double_quotes")) {
		if (is_literal(c1)) {
			if (!slicecmp2(GET_STR(c1), LEN_STR(c1), "atom")) {
				p->flag.double_quote_codes = p->flag.double_quote_chars = false;
				p->flag.double_quote_atom = true;
			} else if (!slicecmp2(GET_STR(c1), LEN_STR(c1), "chars")) {
				p->flag.double_quote_atom = p->flag.double_quote_codes = false;
				p->flag.double_quote_chars = true;
			} else if (!slicecmp2(GET_STR(c1), LEN_STR(c1), "codes")) {
				p->flag.double_quote_atom = p->flag.double_quote_chars = false;
				p->flag.double_quote_codes = true;
			}
		}
	} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "variables")) {
		if (is_variable(c1)) {
			cell *v = c1;
			if (vars) *vars = v;
	...
	} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "variable_names")) {
		if (is_variable(c1)) {
			cell *v = c1;
	...
	} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "singletons")) {
		if (is_variable(c1)) {
			cell *v = c1;
			if (sings) *sings = v;
...
static bool parse_write_params(query *q, cell *c, cell **vnames, idx_t *vnames_ctx)
{
	...
	if (!slicecmp2(GET_STR(c), LEN_STR(c), "max_depth")) {
		...
		q->fullstop = !slicecmp2(GET_STR(c1), LEN_STR(c1), "true");
	} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "nl")) {
		if (!is_literal(c1) || (slicecmp2(GET_STR(c1), LEN_STR(c1), "true") && slicecmp2(GET_STR(c1), LEN_STR(c1), "false"))) {
			DISCARD_RESULT throw_error(q, c, "domain_error", "write_option");
			return false;
		}

		q->nl = !slicecmp2(GET_STR(c1), LEN_STR(c1), "true");
	} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "quoted")) {
		if (!is_literal(c1) || (slicecmp2(GET_STR(c1), LEN_STR(c1), "true") && slicecmp2(GET_STR(c1), LEN_STR(c1), "false"))) {
			DISCARD_RESULT throw_error(q, c, "domain_error", "write_option");
			return false;
		}
		...
		q->ignore_ops = !slicecmp2(GET_STR(c1), LEN_STR(c1), "true");
	} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "numbervars")) {
		if (!is_literal(c1) || (slicecmp2(GET_STR(c1), LEN_STR(c1), "true") && slicecmp2(GET_STR(c1), LEN_STR(c1), "false"))) {
			DISCARD_RESULT throw_error(q, c, "domain_error", "write_option");
			return false;
		}

		q->numbervars = !slicecmp2(GET_STR(c1), LEN_STR(c1), "true");
	} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "variable_names")) {
...
static USE_RESULT pl_status fn_iso_put_code_1(query *q)
{
	...
	if (is_integer(p1) && (get_numerator(p1) <= -1))
		return throw_error(q, p1, "representation_error", "character_code");

	int ch = (int)get_numerator(p1);
	char tmpbuf[20];
	put_char_utf8(tmpbuf, ch);
	net_write(tmpbuf, strlen(tmpbuf), str);
	return !ferror(str->fp);
...
static USE_RESULT pl_status fn_iso_univ_2(query *q)
{
	...
	if (is_variable(p1)) {
		...
		if (search_op(q->st.m, GET_STR(tmp), &specifier, false))
...
static USE_RESULT pl_status fn_iso_abolish_1(query *q)
{
	...
	if (get_builtin(q->st.m->pl, GET_STR(p1_name), get_numerator(p1), &found), found)
...
pl_status throw_error(query *q, cell *c, const char *err_type, const char *expected)
{
	...
	if (is_variable(c)) {
		...
		&& is_structure(c) && slicecmp2(GET_STR(c), LEN_STR(c), "/") && is_variable(c+1)) {
		char tmpbuf[1024];
...
static USE_RESULT pl_status fn_iso_current_predicate_1(query *q)
{
	...
	if (slicecmp2(GET_STR(p_pi), LEN_STR(p_pi), "/"))
		return throw_error(q, p_pi, "type_error", "predicate_indicator");
	...
	if ((!is_integer(p2) || (get_numerator(p2) < 0)) && !is_variable(p2))
		return throw_error(q, p_pi, "type_error", "predicate_indicator");
...
static USE_RESULT pl_status fn_iso_current_prolog_flag_2(query *q)
{
	...
	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "double_quotes")) {
		...
		return unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	} else if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "char_conversion")) {
		cell tmp;
		...
		return unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	} else if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "encoding")) {
		cell tmp;
		...
		return unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	} else if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "character_escapes")) {
		cell tmp;
		...
		return unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	} else if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "integer_rounding_function")) {
		cell tmp;
		...
		return unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	} else if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "version")) {
		unsigned v1 = 0;
		sscanf(VERSION, "v%u", &v1);
		cell tmp;
		make_int(&tmp, v1);
		return unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	} else if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "version_data")) {
		...
		tmp[0].nbr_cells = 5;
		return unify(q, p2, p2_ctx, tmp, q->st.curr_frame);
	} else if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "version_git")) {
		cell tmp;
		make_literal(&tmp, index_from_pool(q->st.m->pl, VERSION));
		return unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	} else if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "argv")) {
		if (g_avc == g_ac) {
			cell tmp;
			make_literal(&tmp, g_nil_s);
			return unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
		...
		return ok;
	} else if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "unknown")) {
		cell tmp;
		make_literal(&tmp,
...
static USE_RESULT pl_status fn_iso_set_prolog_flag_2(query *q)
{
	...
	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "double_quotes")) {
		...
		else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "error"))
			q->st.m->flag.occurs_check = -1;
		else {
			cell *tmp = alloc_on_heap(q, 3);
			make_structure(tmp, g_plus_s, fn_iso_add_2, 2, 2);
		...
		if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "fail")) {
			q->st.m->flag.unknown = UNK_FAIL;
		} else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "error")) {
			q->st.m->flag.unknown = UNK_ERROR;
		} else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "warning")) {
		...
		|| !slicecmp2(GET_STR(p1), LEN_STR(p1), "max_integer")
		|| !slicecmp2(GET_STR(p1), LEN_STR(p1), "min_integer")
...
static USE_RESULT pl_status fn_sys_queuen_2(query *q)
{
	...
	alloc_on_queuen(q, get_numerator(p1), tmp);
	return pl_success;
}
...
{
	while (is_structure(p2) && !slicecmp2(GET_STR(p2), LEN_STR(p2), "^")) {
		cell *c = ++p2;
...
static pl_status do_op(query *q, cell *p3)
{
	...
	if (pri && !slicecmp2(GET_STR(p3), LEN_STR(p3), "|") && (!IS_INFIX(specifier) || (pri < 1001)))
...
static USE_RESULT pl_status fn_iso_op_3(query *q)
{
	...
	if (is_integer(p1) && ((get_numerator(p1) < 0) || (get_numerator(p1) > 1200)))
...
static USE_RESULT pl_status fn_listing_1(query *q)
{
	...
	if (p1->arity) {
		if (slicecmp2(GET_STR(p1), LEN_STR(p1), "/") && slicecmp2(GET_STR(p1), LEN_STR(p1), "//"))
			return throw_error(q, p1, "type_error", "predicate_indicator");
...
static USE_RESULT pl_status fn_statistics_2(query *q)
{
	...
	if (!slicecmp2(GET_STR(p1), LEN_STR(p1), "gctime") && is_variable(p2)) {
		cell tmp;
		make_float(&tmp, 0);
		set_var(q, p2, p2_ctx, &tmp, q->st.curr_frame);
		return pl_success;
...
static USE_RESULT pl_status fn_sleep_1(query *q)
{
	...
	if (q->is_task) {
		do_yield_0(q, get_numerator(p1)*1000);
		return pl_failure;
	}

	sleep((unsigned)get_numerator(p1));
	return pl_success;
}
...
static USE_RESULT pl_status fn_delay_1(query *q)
{
	...
	if (q->is_task) {
		do_yield_0(q, get_numerator(p1));
		return pl_failure;
	}

	msleep((unsigned)get_numerator(p1));
	return pl_success;
}
...
static USE_RESULT pl_status fn_server_3(query *q)
{
	...
	while (is_list(p3)) {
		...
		if (is_structure(c) && (c->arity == 1)) {
			if (!slicecmp2(GET_STR(c), LEN_STR(c), "udp")) {
				c = c + 1;

				if (is_atom(c))
					udp = !slicecmp2(GET_STR(c), LEN_STR(c), "true") ? 1 : 0;
			} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "nodelay")) {
				c = c + 1;

				if (is_atom(c))
					nodelay = !slicecmp2(GET_STR(c), LEN_STR(c), "true") ? 1 : 0;
			} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "ssl")) {
				c = c + 1;

				if (is_atom(c))
					ssl = !slicecmp2(GET_STR(c), LEN_STR(c), "true") ? 1 : 0;
			} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "keyfile")) {
				c = c + 1;

				if (is_atom(c))
					keyfile = GET_STR(c);
			} else if (!slicecmp2(GET_STR(c), LEN_STR(c), "certfile")) {
				c = c + 1;
				...
				if (is_atom(c)) {
					ssl = !slicecmp2(GET_STR(c), LEN_STR(c), "https") ? 1 : 0;
					port = 443;
				}
...
static USE_RESULT pl_status fn_client_5(query *q)
{
	...
	while (is_list(p5)) {
		...
		if (is_structure(c) && (c->arity == 1)) {
			if (!slicecmp2(GET_STR(c), LEN_STR(c), "host")) {
				c = c + 1;
...
static USE_RESULT pl_status fn_bread_3(query *q)
{
	...
	if (is_integer(p1) && (get_numerator(p1) > 0)) {
		if (!str->data) {
			str->data = malloc(get_numerator(p1)+1);
			may_ptr_error(str->data);
			str->data_len = 0;
		}

		for (;;) {
			len = get_numerator(p1) - str->data_len;
...
static pl_status do_format(query *q, cell *str, idx_t str_ctx, cell* p1, cell* p2, idx_t p2_ctx)
{
	...
	*dst = '\0';
	...
	} else if (is_structure(str) && ((slicecmp2(GET_STR(str), LEN_STR(str), "atom") && slicecmp2(GET_STR(str), LEN_STR(str), "chars") && slicecmp2(GET_STR(str), LEN_STR(str), "string")) || (str->arity > 1) || !is_variable(str+1))) {
		free(tmpbuf);
		return throw_error(q, c, "type_error", "structure");
	} else if (is_structure(str) && !slicecmp2(GET_STR(str), LEN_STR(str), "atom")) {
...
static USE_RESULT pl_status fn_hex_chars_2(query *q)
{
	...
	if (is_variable(p1)) {
		char tmpbuf[256];
		snprintf(tmpbuf, sizeof(tmpbuf), "%llx", (long long)get_numerator(p2));
		cell tmp;
		may_error(make_string(&tmp, tmpbuf));
		set_var(q, p1, p1_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
	...
	return p1_val == get_numerator(p2);
}
...
static USE_RESULT pl_status fn_octal_chars_2(query *q)
{
	...
	if (is_variable(p1)) {
		char tmpbuf[256];
		snprintf(tmpbuf, sizeof(tmpbuf), "%llo", (long long)get_numerator(p2));
		cell tmp;
		may_error(make_string(&tmp, tmpbuf));
		set_var(q, p1, p1_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
...
static USE_RESULT pl_status fn_setenv_2(query *q)
{
	...
	if (is_atom(p2)) {
		...
		char tmpbuf[256];
		sprint_int(tmpbuf, sizeof(tmpbuf), get_numerator(p2), 10);
...
static USE_RESULT pl_status fn_numbervars_3(query *q)
{
	...
	GET_NEXT_ARG(p2,integer);
	GET_NEXT_ARG(p3,integer_or_var);
	int end = q->nv_start = get_numerator(p2);
	unsigned cnt = real_numbervars(q, p1, p1_ctx, &end);
	cell tmp;
	make_int(&tmp, get_numerator(p2)+cnt);
	return unify(q, p3, p3_ctx, &tmp, q->st.curr_frame);
}
...
static USE_RESULT pl_status fn_char_type_2(query *q)
{
	...
	if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "alpha"))
		return isalpha(ch);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "digit"))
		return isdigit(ch);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "xdigit"))
		return isxdigit(ch);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "whitespace"))
		return isblank(ch) || iswspace(ch);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "white"))
		return isblank(ch);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "space"))
		return iswspace(ch);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "lower"))
		return islower(ch);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "upper"))
		return isupper(ch);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "punct"))
		return ispunct(ch);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "cntrl"))
		return iscntrl(ch);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "graph"))
		return isgraph(ch);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "ascii"))
		return ch < 128;
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "newline"))
		return ch == 10;
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "end_of_line"))
		return (ch >= 10) && (ch <= 13);
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "end_of_file"))
		return ch == -1;
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "quote"))
		return (ch == '\'') || (ch == '"') || (ch == '`');
	else if (!slicecmp2(GET_STR(p2), LEN_STR(p2), "period"))
		return (ch == '.') || (ch == '!') || (ch == '?');
...
static USE_RESULT pl_status fn_succ_2(query *q)
{
	...
	if (is_variable(p2)) {
		cell tmp;
		make_int(&tmp, get_numerator(p1)+1);
		return unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	}
	...
	if (is_variable(p1)) {
		...
		cell tmp;
		make_int(&tmp, get_numerator(p2)-1);
		return unify(q, p1, p1_ctx, &tmp, q->st.curr_frame);
	}

	return get_numerator(p1) == (get_numerator(p2) - 1);
}
...
static USE_RESULT pl_status fn_plus_3(query *q)
{
	...
	if (is_variable(p3)) {
		...
		cell tmp;
		make_int(&tmp, get_numerator(p1) + get_numerator(p2));
		set_var(q, p3, p3_ctx, &tmp, q->st.curr_frame);
		return pl_success;
	}
...
static USE_RESULT pl_status fn_limit_2(query *q)
{
	...
	make_int(tmp+nbr_cells++, get_numerator(p1));
	make_call(q, tmp+nbr_cells);
	q->st.curr_cell = tmp;
	return pl_success;
}
...
static USE_RESULT pl_status fn_call_nth_2(query *q)
{
	...
	make_int(tmp+nbr_cells++, 1);
	make_int(tmp+nbr_cells++, get_numerator(p2));
	make_call(q, tmp+nbr_cells);
	q->st.curr_cell = tmp;
	return pl_success;
}
...
static USE_RESULT pl_status fn_iso_length_2(query *q)
{
	...
	if (is_variable(p1) && is_integer(p2)) {
		...
		if (get_numerator(p2) >= MAX_VARS)
			return throw_error(q, p2, "resource_error", "too_many_vars");

		idx_t nbr = get_numerator(p2);
...
static USE_RESULT pl_status fn_kv_set_3(query *q)
{
	...
	if (is_integer(p1)) {
		char tmpbuf[128];
		snprintf(tmpbuf, sizeof(tmpbuf), "%lld", (long long unsigned)get_numerator(p1));
		key = strdup(tmpbuf);
	} else if (is_atom(p1))
	...
	if (is_integer(p2)) {
		char tmpbuf[128];
		snprintf(tmpbuf, sizeof(tmpbuf), "%lld", (long long unsigned)get_numerator(p2));
		val = strdup(tmpbuf);
	} else if (is_atom(p2))
...
static USE_RESULT pl_status fn_iso_compare_3(query *q)
{
	...
	if (is_atom(p1)) {
		if (slicecmp2(GET_STR(p1), LEN_STR(p1), "<")
			&& slicecmp2(GET_STR(p1), LEN_STR(p1), ">")
			&& slicecmp2(GET_STR(p1), LEN_STR(p1), "="))
...
static void load_properties(module *m)
{
	...
	format_property(tmpbuf, sizeof(tmpbuf), ",", 2, "meta_predicate((0,0))"); STRING_strcat(pr, tmpbuf);
	...
	format_property(tmpbuf, sizeof(tmpbuf), "findall", 3, "control_construct"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "findall", 3, "meta_predicate(findall(?,0,-))"); STRING_strcat(pr, tmpbuf);

	format_property(tmpbuf, sizeof(tmpbuf), "bagof", 3, "control_construct"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "bagof", 3, "meta_predicate(bagof(?,0,-))"); STRING_strcat(pr, tmpbuf);

	format_property(tmpbuf, sizeof(tmpbuf), "setof", 3, "control_construct"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "setof", 3, "meta_predicate(setof(?,0,-))"); STRING_strcat(pr, tmpbuf);

	format_property(tmpbuf, sizeof(tmpbuf), "throw", 1, "control_construct"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "call", 1, "control_construct"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "!", 0, "control_construct"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "true", 0, "control_construct"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "fail", 0, "control_construct"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "|", 2, "meta_predicate((:|+))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "time", 1, "meta_predicate(time(0))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "setup_call_cleanup", 3, "meta_predicate(setup_call_cleanup(0,0,0))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "asserta", 1, "meta_predicate(asserta(:))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "assertz", 1, "meta_predicate(assertz(:))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "retract", 1, "meta_predicate(retract(:))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "retractall", 1, "meta_predicate(retractall(:))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "current_predicate", 1, "meta_predicate(current_predicate(:))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "predicate_property", 1, "meta_predicate(predicate_property(:,?))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "abolish", 1, "meta_predicate(abolish(:))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "clause", 2, "meta_predicate(abolish(:,?))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "catch", 3, "meta_predicate(catch(0,?,0))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "phrase", 2, "meta_predicate(phrase(2,?))"); STRING_strcat(pr, tmpbuf);
	format_property(tmpbuf, sizeof(tmpbuf), "phrase", 3, "meta_predicate(phrase(2,?,?))"); STRING_strcat(pr, tmpbuf);
	...
	for (int i = 2; i <= 7; i++) {
		...
		format_property(tmpbuf, sizeof(tmpbuf), "call", i, metabuf); STRING_strcat(pr, tmpbuf);
	}
	...
	for (int i = 2; i <= 7; i++) {
		...
		snprintf(dst2, sizeof(metabuf)-(dst2-metabuf), "))");
		format_property(tmpbuf, sizeof(tmpbuf), "task", i, metabuf); STRING_strcat(pr, tmpbuf);
	}

	for (const struct builtins *ptr = g_predicates_iso; ptr->name; ptr++) {
		m_app(m->pl->funtab, ptr->name, ptr);
		if (ptr->name[0] == '$') continue;
		format_property(tmpbuf, sizeof(tmpbuf), ptr->name, ptr->arity, "built_in"); STRING_strcat(pr, tmpbuf);
		format_property(tmpbuf, sizeof(tmpbuf), ptr->name, ptr->arity, "static"); STRING_strcat(pr, tmpbuf);
		format_property(tmpbuf, sizeof(tmpbuf), ptr->name, ptr->arity, "private"); STRING_strcat(pr, tmpbuf);
		format_property(tmpbuf, sizeof(tmpbuf), ptr->name, ptr->arity, "native_code"); STRING_strcat(pr, tmpbuf);
 	}
