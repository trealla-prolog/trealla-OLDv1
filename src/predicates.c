...
static USE_RESULT pl_status fn_iso_get_code_1(query *q)
{
	...
	if (is_smallint(p1) && (get_integer(p1) < -1))
...
static USE_RESULT pl_status fn_succ_2(query *q)
{
	...
	if (is_variable(p1)) {
		...
		cell tmp;
		make_int(&tmp, get_integer(p2)-1);
		return unify(q, p1, p1_ctx, &tmp, q->st.curr_frame);
	}

	return get_integer(p1) == (get_integer(p2) - 1);
}
...
static USE_RESULT pl_status fn_plus_3(query *q)
{
	...
	if (is_variable(p1)) {
		...
		cell tmp;
		make_int(&tmp, get_integer(p3) - get_integer(p2));
		set_var(q, p1, p1_ctx, &tmp, q->st.curr_frame);
		return pl_success;
	}
	...
	if (is_variable(p2)) {
		...
		cell tmp;
		make_int(&tmp, get_integer(p3) - get_integer(p1));
		set_var(q, p2, p2_ctx, &tmp, q->st.curr_frame);
		return pl_success;
	}
	...
	if (is_variable(p3)) {
		...
		cell tmp;
		make_int(&tmp, get_integer(p1) + get_integer(p2));
		set_var(q, p3, p3_ctx, &tmp, q->st.curr_frame);
		return pl_success;
	}

	return get_integer(p3) == get_integer(p1) + get_integer(p2);
}
