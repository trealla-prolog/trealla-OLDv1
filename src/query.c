...
{
	may_heap_error(check_slot(q, MAX_ARITY));
...
USE_RESULT bool match_rule(query *q, cell *p1, pl_idx_t p1_ctx)
{
	...
	for (; q->st.curr_clause2; q->st.curr_clause2 = q->st.curr_clause2->next) {
		...
		may_heap_error(try_me(q, cl->nbr_vars));
	...
	while (!done && !q->error) {
		...
		if (is_builtin(q->st.curr_cell)) {
			...
			if (q->run_hook && !q->in_hook)
				may_heap_error(do_post_unification_hook(q, true));
			...
			if (q->run_hook && !q->in_hook) {
				may_heap_error(do_post_unification_hook(q, false));
			}
		}
