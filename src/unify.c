...
static void make_new_variable(query *q, cell *tmp, unsigned var_nbr, pl_idx_t ctx)
{
...
static void set_new_variable(query *q, cell *tmp, cell *v, pl_idx_t ctx)
{
	make_variable(tmp, g_anon_s, create_vars(q, 1));
	set_var(q, tmp, q->st.curr_frame, v, ctx);
}
...
pl_status fn_sys_undo_trail_1(query *q)
{
	...
	for (pl_idx_t i = q->undo_lo_tp, j = 0; i < q->undo_hi_tp; i++, j++) {
		...
		cell lhs, rhs;
		make_new_variable(q, &lhs, tr->var_nbr, tr->ctx);
		set_new_variable(q, &rhs, &e->c, e->ctx);
