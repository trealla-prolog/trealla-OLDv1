...
static bool do_stream_property(query *q)
{
	...
	if (!CMP_STR_CSTR(q, p1, "file_name")) {
		cell tmp;
		may_heap_error(make_cstring(&tmp, str->filename));
	...
	if (!CMP_STR_CSTR(q, p1, "alias")) {
		cell tmp;
		may_heap_error(make_cstring(&tmp, str->name));
		bool ok = unify(q, c, c_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		return ok;
	}
	...
	if (!CMP_STR_CSTR(q, p1, "mode")) {
		cell tmp;
		may_heap_error(make_cstring(&tmp, str->mode));
		bool ok = unify(q, c, c_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		return ok;
	}
	...
	if (!CMP_STR_CSTR(q, p1, "bom") && !str->binary) {
		cell tmp;
		may_heap_error(make_cstring(&tmp, str->bom?"true":"false"));
		bool ok = unify(q, c, c_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		return ok;
	}
	...
	if (!CMP_STR_CSTR(q, p1, "type")) {
		cell tmp;
		may_heap_error(make_cstring(&tmp, str->binary ? "binary" : "text"));
		bool ok = unify(q, c, c_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		return ok;
	}
	...
	if (!CMP_STR_CSTR(q, p1, "reposition")) {
		cell tmp;
		may_heap_error(make_cstring(&tmp, str->socket || (n <= 2) ? "false" : "true"));
		bool ok = unify(q, c, c_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		return ok;
	}
	...
	if (!CMP_STR_CSTR(q, p1, "encoding") && !str->binary) {
		cell tmp;
		may_heap_error(make_cstring(&tmp, "UTF-8"));
		bool ok = unify(q, c, c_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		return ok;
	}
	...
	if (!CMP_STR_CSTR(q, p1, "newline")) {
		cell tmp;
		may_heap_error(make_cstring(&tmp, NEWLINE_MODE));
		bool ok = unify(q, c, c_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		return ok;
	}
...
static USE_RESULT bool fn_popen_4(query *q)
{
	...
	stream *str = &q->pl->streams[n];
	str->domain = true;
	may_heap_error(str->filename = strdup(filename));
	may_heap_error(str->name = strdup(filename));
	may_heap_error(str->mode = DUP_STR(q, p2));
...
static USE_RESULT bool fn_write_term_to_atom_3(query *q)
{
	...
	clear_write_options(q);
	cell tmp;
	may_heap_error(make_cstring(&tmp, dst), free(dst));
	free(dst);
...
static USE_RESULT bool fn_write_term_to_chars_3(query *q)
{
	...
	clear_write_options(q);
	cell tmp;
	may_heap_error(make_string(&tmp, dst), free(dst));
	free(dst);
	bool ok = unify(q, p_chars, p_chars_ctx, &tmp, q->st.curr_frame);
	unshare_cell(&tmp);
	return ok;
...
static USE_RESULT bool fn_edin_seeing_1(query *q)
{
	...
	cell tmp;
	may_heap_error(make_cstring(&tmp, name));
...
static USE_RESULT bool fn_read_line_to_string_2(query *q)
{
	...
	cell tmp;
	may_heap_error(make_string(&tmp, line), free(line));
	free(line);
	bool ok = unify(q, p1, p1_ctx, &tmp, q->st.curr_frame);
	unshare_cell(&tmp);
	return ok;
...
static USE_RESULT bool fn_read_file_to_string_3(query *q)
{
	...
	fclose(fp);
	cell tmp;
	may_heap_error(make_stringn(&tmp, s, len), free(s));
...
static USE_RESULT bool fn_load_files_2(query *q)
{
	...
	if (is_atom(p1)) {
		may_heap_error(do_consult(q, p1, p1_ctx));
		return true;
	}
	...
	while (is_list(p1)) {
		...
		pl_idx_t c_ctx = q->latest_ctx;
		may_heap_error(do_consult(q, c, c_ctx));
		p1 = LIST_TAIL(p1);
		p1 = deref(q, p1, p1_ctx);
		p1_ctx = q->latest_ctx;
	}
...
static USE_RESULT bool fn_unload_files_1(query *q)
{
	...
	if (is_atom(p1)) {
		may_heap_error(do_deconsult(q, p1, p1_ctx));
		return true;
	}
	...
	while (is_list(p1)) {
		...
		cell *c = deref(q, h, p1_ctx);
		pl_idx_t c_ctx = q->latest_ctx;
		may_heap_error(do_deconsult(q, c, c_ctx));
		p1 = LIST_TAIL(p1);
		p1 = deref(q, p1, p1_ctx);
		p1_ctx = q->latest_ctx;
	}
...
static USE_RESULT bool fn_getfile_2(query *q)
{
	...
	while (getline(&line, &len, fp) != -1) {
		...
		cell tmp;
		may_heap_error(make_stringn(&tmp, line, len));
...
static USE_RESULT bool fn_absolute_file_name_3(query *q)
{
		...
		*dst = '\0';
		...
		if ((tmpbuf = realpath(s, NULL)) == NULL) {
			...
			if ((*s != '/') && (*s != '\\')
				...
				may_heap_error(tmp, free(tmpbuf));
	...
	if (is_string(p1))
		may_heap_error(make_string(&tmp, tmpbuf), free(tmpbuf));
	else
		may_heap_error(make_cstring(&tmp, tmpbuf), free(tmpbuf));
...
static USE_RESULT bool fn_directory_files_2(query *q)
{
	...
	if (is_string(p1))
		may_heap_error(make_string(&tmp, dire->d_name));
	else
		may_heap_error(make_cstring(&tmp, dire->d_name));
...
static USE_RESULT bool fn_working_directory_2(query *q)
{
	...
	oldpath = tmpbuf2;
	cell tmp;
	may_heap_error(make_string(&tmp, oldpath));
...
static USE_RESULT bool fn_server_3(query *q)
{
	...
	stream *str = &q->pl->streams[n];
	may_heap_error(str->filename = DUP_STR(q, p1));
	may_heap_error(str->name = strdup(hostname));
	may_heap_error(str->mode = strdup("update"));
	str->nodelay = nodelay;
...
static USE_RESULT bool fn_accept_2(query *q)
{
	...
	may_heap_error(str2->filename = strdup(str->filename));
	may_heap_error(str2->name = strdup(str->name));
	may_heap_error(str2->mode = strdup("update"));
	str->socket = true;
...
static USE_RESULT bool fn_client_5(query *q)
{
	...
	if (str->ssl) {
		...
		may_heap_error (str->sslptr, close(fd));
	}
	...
	cell tmp;
	may_heap_error(make_string(&tmp, hostname));
	set_var(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	unshare_cell(&tmp);
	may_heap_error(make_string(&tmp, path));
	set_var(q, p3, p3_ctx, &tmp, q->st.curr_frame);
	unshare_cell(&tmp);
	cell tmp2;
	make_int(&tmp2, n);
...
static USE_RESULT bool fn_bread_3(query *q)
{
	...
	if (is_integer(p1) && is_positive(p1)) {
		...
		cell tmp;
		may_heap_error(make_stringn(&tmp, str->data, str->data_len), free(str->data));
		set_var(q, p2, p2_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		free(str->data);
		str->data = NULL;
	...
	if (is_integer(p1)) {
		...
		may_heap_error(str->data);
		cell tmp;
		may_heap_error(make_stringn(&tmp, str->data, nbytes), free(str->data));
		set_var(q, p2, p2_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		free(str->data);
		str->data = NULL;
	...
	if (str->data_len)
		may_heap_error(make_stringn(&tmp2, str->data, str->data_len), free(str->data));
	else
