...
USE_RESULT pl_status fn_sys_soft_cut_0(query *q)
{
	cut_me(q, true, true);
	return pl_success;
}
...
pl_status throw_error3(query *q, cell *c, UNUSED pl_idx_t c_ctx, const char *err_type, const char *expected, cell *goal)
{
	...
	if (is_variable(c)) {
		...
		tmp = alloc_on_tmp(q, 6+(c->nbr_cells-1));
		may_ptr_error(tmp);
		pl_idx_t nbr_cells = 0;
		...
		tmp = alloc_on_tmp(q, 7+(c->nbr_cells-1)+extra);
		may_ptr_error(tmp);
		pl_idx_t nbr_cells = 0;
